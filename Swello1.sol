...
contract Swello is ERC20Detailed, Ownable, ReentrancyGuard {
    ...
    uint256 public rewardYield = 2081456;   // yielding amount every 15 min = 0.0002081456283510475789732219941
    uint256 public rewardYieldDenominator = 100000000000;

    uint256 public rebaseFrequency = 900; // every 15 min = 15 * 60s
    uint256 public nextRebase = block.timestamp + 31536000; // 365 days = 365 * 24 * 3600

    uint256 public autoLPFrequency = 172800; // every 48 hours = 48 * 3600s
    uint256 public nextAutoLP = block.timestamp + 31536000; // 365 days = 365 * 24 * 3600
    ...
    uint256 public safetyFundFee = 5;
    uint256 public sellFeeliquidityAdded = 5;
    uint256 public totalBuyFee = liquidityFee.add(treasuryFee).add(safetyFundFee);
    uint256 public totalSellFee = totalBuyFee.add(sellFeeliquidityAdded);
    
    uint256 []bonfireBalanceThresholds = [100000, 250000, 500000, 1000000, 2000000, 3000000, 4000000, 5000000];
    uint256 []bonfireFees = [1, 2, 3, 4, 5, 6, 7, 8];

    uint256 public feeDenominator = 100;

    uint256 public bonfireBurn = 60;
    uint256 public bonfireTreasury = 30;
    uint256 public bonfireCharity = 10;
    uint256 public bonfireDenominator = bonfireBurn.add(bonfireTreasury).add(bonfireCharity);
    ...
    ) ERC20Detailed("Swello", "SWELLO", uint8(DECIMALS))
    {
